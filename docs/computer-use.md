...
| Linux Wayland x64/arm64 | AT-SPI accessibility plus compositor-native fallbacks: `grim` screen capture (preferred when on `PATH`: no portal dialog, no PipeWire round-trip), `niri msg` window listing/focus on Niri, and `wtype` keyboard input when the RemoteDesktop portal or `LIBEI_SOCKET` is unavailable. The ScreenCast portal/PipeWire path (builds compiled with the `wayland-pipewire` Cargo feature) is used when `grim` is absent or fails. `capabilities()` reflects whichever path is actually usable. RemoteDesktop permission is requested lazily on first native input, is not persisted, and closes with the desktop session; read-only window/AX inspection does not request it. Compositor restrictions apply; background per-window native input is unavailable. |
...
Inspect `computer.capabilities()` rather than assuming capture, input, AX, or permission state. On Wayland, input reports `prompt-or-granted` before first native input without opening a RemoteDesktop session. Capture reports `true` when the `wayland-pipewire` feature is compiled in or `grim` is on `PATH`; when neither is present — or the RemoteDesktop portal denies input and `wtype` is missing — the failure surfaces as a capture/input/permission error rather than falling back to X11.
